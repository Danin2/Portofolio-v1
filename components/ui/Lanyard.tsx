...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: BandProps) {
  ...
  // Titik tempel tali ke pengait. 
  // Karena kartu kita perbesar (scale 3.25), posisi klip secara visual naik ke atas.
  // Nilai Y ini (2.65) didapat dari perhitungan rasio ukuran agar pas dengan klip di ukuran baru.
  const stringAttachmentPoint: [number, number, number] = [0, 2.65, 0]; 
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      if (band.current) {
        ...
        band.current.geometry = new THREE.TubeGeometry(curve, isMobile ? 24 : 64, 0.05, 8, false);
      }
  ...
  return (
    <>
      <group position={[0, 4, 0]}>
        ...
        >
          {/* Ukuran fisik kotak collider juga dinaikkan agar proporsional dengan skala baru */}
          <CuboidCollider args={[1.5, 2.7, 0.01]} />
          <group
            scale={3.25} 
            position={[0, -1.2, -0.05]}
