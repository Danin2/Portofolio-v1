    ...
    <div className="rounded-[2rem] p-8 border border-[var(--border-primary)] bg-[var(--bg-secondary)]/40 backdrop-blur-xl shadow-xl group hover:border-[var(--accent-primary)]/30 transition-all duration-500">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-[var(--text-primary)]">{t('contact.availability_status')}</span>
        </div>
      </div>

      <h3 className="text-xl font-black text-[var(--text-primary)] mb-2">{t('contact.availability_title')}</h3>
      ...
      <div className="space-y-3 border-t border-[var(--border-primary)] pt-6">
        <div className="flex items-center justify-between text-[0.65rem]">
          <span className="text-[var(--text-muted)] font-bold uppercase tracking-widest">{t('contact.timezone_label')}</span>
          <span className="text-[var(--text-primary)] font-mono">WIB (UTC+7)</span>
        </div>
        <div className="flex items-center justify-between text-[0.65rem]">
          <span className="text-[var(--text-muted)] font-bold uppercase tracking-widest">{t('contact.response_label')}</span>
...
export default function ContactClient() {
  ...
  // Build contact info items reactively
  const contactInfoItems = [
    {
      label: (t('contact.contact_items') as any[])?.[0]?.label ?? 'Primary Email',
      value: 'danindra@danindra.dev',
      icon: <MailIcon />,
      href: 'mailto:danindra@danindra.dev',
      helper: (t('contact.contact_items') as any[])?.[0]?.helper ?? 'Direct transmission line',
    },
    {
      label: (t('contact.contact_items') as any[])?.[1]?.label ?? 'Connect',
      value: 'linkedin.com/in/danindra',
      icon: <LinkedInIcon />,
      href: 'https://linkedin.com/in/danindra',
      helper: (t('contact.contact_items') as any[])?.[1]?.helper ?? 'Professional network',
    },
    {
          ...
          <div className="lg:col-span-2 space-y-16">
            <div className="space-y-8">
              ...
              >
                ...
                <span className="gradient-text italic font-medium">{t('contact.title_line2')}</span>
              </RevealText>
              ...
              {contactInfoItems.map((item, idx) => {
                const contentInner = (
                  <div className="flex items-center gap-6 shrink-0">
                    ...
                    <div>
                      <p className="text-[0.6rem] uppercase tracking-[0.25em] text-[var(--text-secondary)] font-black mb-1 flex items-center gap-2">
                        {item.label}
                      ...
                      <p className="text-[var(--text-primary)] font-bold text-base md:text-lg group-hover:text-[var(--accent-primary)] transition-colors duration-300 break-all">{item.value}</p>
                      <p className="text-xs font-mono text-[var(--text-secondary)] mt-1 opacity-0 group-hover:opacity-100 transition-opacity">{item.helper}</p>
                    </div>
                  </div>
                );
            ...
            <ScrollReveal delay={0.8}>
              <div className="space-y-4 pt-4">
                <p className="text-[0.65rem] font-black uppercase tracking-[0.3em] text-[var(--text-muted)]">{t('contact.terminal_nodes')}</p>
                  ...
                  <AnimatePresence mode="wait">
                    {formStatus === 'success' ? (
                      ...
                      >
                        ...
                        <h3 className="text-3xl font-black text-[var(--text-primary)] mb-4 tracking-tight">{t('contact.success_title')}</h3>
                        <p className="text-[var(--text-secondary)] mb-10 font-medium max-w-sm mx-auto">{t('contact.success_desc')}</p>
                        <button
                      ...
                      >
                        ...
                        >
                          <span className="relative z-10 flex items-center justify-center gap-3">
                            {formStatus === 'sending' ? t('contact.submitting_btn') : (
                              <>
